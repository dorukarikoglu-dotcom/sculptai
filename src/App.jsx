...
/* ─── ML SİSTEMİ — ağırlıklar ve skorlama server-side (netlify/functions/score-patient.js) ── */
const V6B_THRESHOLD = 48; // Optimal F1 threshold for no-conv class (classify için)
...
function PatientForm({doctorId}){
  ...
  async function handleSubmit(){
    ...
    setSubmitting(true);
    setSubmitError("");
    // Skorlama server-side yapılır — model ağırlıkları client'ta yok.
    // Yalnızca skorlamada kullanılan alanlar gönderilir (PII gönderilmez).
    const scoringAnswers={
      revision:answers.revision,riskKnowledge:answers.riskKnowledge,prevSurgery:answers.prevSurgery,
      procedure:answers.procedure,motivation:answers.motivation,support:answers.support,
    ...
    for(let attempt=0;attempt<3;attempt++){
      try{
        const res=await fetch("/api/score-patient",{
          ...
          body:JSON.stringify({doctor_id:doctorId,answers:scoringAnswers}),
        });
        if(res.ok){
